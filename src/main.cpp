...
void loop()
{
  ...
  case player_white:
    if (millis() - timer > 995)
    { // Display the white player clock
      countdown();
      lcd_display();
    }
    ...
    case player_black:
      //  Game mode HvsH
      if (game_mode == HvsH)
      { // Display the black player clock
        if (millis() - timer > 995)
        {
          countdown();
          lcd_display();
...
void motor(byte direction, int speed, float distance)
{
  ...
  if (distance == calibrate_speed)
    step_number = 4;
  else if (direction == LR_BT || direction == RL_TB || direction == LR_TB || direction == RL_BT)
    step_number = distance * SQUARE_SIZE * 1.44; //  Add an extra length for the diagonal
  else
    step_number = distance * SQUARE_SIZE;

  //  Direction of the motor rotation
  if (direction == R_L || direction == T_B || direction == RL_TB)
    digitalWrite(MOTOR_WHITE_DIR, HIGH);
  else
    digitalWrite(MOTOR_WHITE_DIR, LOW);
  if (direction == B_T || direction == R_L || direction == RL_BT)
...
void black_player_movement()
{
  ...
  for (byte i = white_capturing; i < 2; i++)
  {
    ...
    if (departure_coord_X > trolley_coordinate_X)
      motor(T_B, SPEED_FAST, displacement_X);
    else if (departure_coord_X < trolley_coordinate_X)
      motor(B_T, SPEED_FAST, displacement_X);
    if (departure_coord_Y > trolley_coordinate_Y)
      motor(L_R, SPEED_FAST, displacement_Y);
    else if (departure_coord_Y < trolley_coordinate_Y)
      motor(R_L, SPEED_FAST, displacement_Y);
    if (i == 0)
    {
      electromagnet(true);
  ...
  if ((displacement_X == 1 && displacement_Y == 2) || (displacement_X == 2 && displacement_Y == 1))
  {
    if (displacement_Y == 2)
    {
      if (departure_coord_X < arrival_coord_X)
      {
        ...
        if (departure_coord_Y < arrival_coord_Y)
          motor(L_R, SPEED_SLOW, displacement_Y);
        else
          motor(R_L, SPEED_SLOW, displacement_Y);
        motor(T_B, SPEED_SLOW, displacement_X * 0.5);
      }
    ...
    else if (displacement_X == 2)
    {
      if (departure_coord_Y < arrival_coord_Y)
      {
        ...
        if (departure_coord_X < arrival_coord_X)
          motor(T_B, SPEED_SLOW, displacement_X);
        else
          motor(B_T, SPEED_SLOW, displacement_X);
        motor(L_R, SPEED_SLOW, displacement_Y * 0.5);
      }
